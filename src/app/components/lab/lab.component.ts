...
export class LabComponent implements OnInit {
  ...
  ngOnInit(): void
  {
    ...
    this.service.getSubjectsInfo().subscribe(data =>{
      ...
      this.labOpis = this.subject[0].lab;
    });
    ...
    this.service.getFiles().subscribe(data=>{
      data.forEach(element => {
        if(element.subjectName == k && element.type=="Lab")
        {
  ...
  saveLab()
  {
    ...
    {
      this.subject.forEach(data => {
        data.lab = this.labOpis;
        this.service.updateSubject(data).subscribe(data => {
          alert("Successfully updated lab description!");
        });
      })
    }
  }
